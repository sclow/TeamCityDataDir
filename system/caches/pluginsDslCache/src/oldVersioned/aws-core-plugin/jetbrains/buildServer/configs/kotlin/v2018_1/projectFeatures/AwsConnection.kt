 ...
 * When using Session Credentials a new temporary Access Key will be ge created each time this connection is used.
 * ```
 * awsConnection {
 *   id = "AwsIamUser"
 *   name = "AWS IAM User"
 ...
 *
 * **Example.**
 * Adds a new Connection that allows TeamCity to assume AWS IAM Role using another AWS Connection.
 * ```
 * awsConnection {
 *   id = "AwsIamRole"
 *   name = "AWS IAM Role"
 ...
 *
 * **Example.**
 * Adds a new Connection that allows TeamCity to extract credentials from default places on the TeamCity server machine using the Default Credentials Provider Chain.
 * ```
 * awsConnection {
